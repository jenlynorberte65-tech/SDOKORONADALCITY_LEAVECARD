...
export default function LeaveCardsPage({ onOpenCard }: Props) {
  ...
  async function doMonthlyNTAccrual() {
    ...
    if (errors.length > 0) {
      ...
      alert(`✅ Monthly accrual successfully posted!\n\n• ${successCount} Non-Teaching employee(s) received 1.25 (Set A) + 1.25 (Set B)\n• Month: ${monthLabel}`);
    }
    setAccrualMsg(accrualDone ? '' : `✅ Posted for ${monthLabel}`);
  }
      ...
      <div className="toolbar no-print" style={{ flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            id="ntAccrualBtn"
            className="btn"
            style={{ background: 'linear-gradient(135deg,#1a5c42,#2e7d52)', color: 'white', fontWeight: 700, fontSize: 12, height: 36, padding: '0 18px', borderRadius: 8, opacity: accrualDone ? 0.5 : 1, cursor: accrualDone ? 'not-allowed' : 'pointer' }}
            onClick={doMonthlyNTAccrual}
            disabled={accrualPosting || accrualDone}
          >
            {accrualPosting ? '⏳ Posting…' : '📈 Post Monthly NT Accrual (1.25 each)'}
          </button>
            ...
            {sorted.map(e => {
              ...
              return (
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 8, border: '1.5px solid var(--br)', background: 'var(--cd)', cursor: 'pointer', fontFamily: 'Inter,sans-serif', fontSize: 12, fontWeight: 500, transition: 'all .15s' }}
                >
                  <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, fontWeight: 700, background: isT ? '#ddeeff' : 'var(--g4)', color: isT ? 'var(--nb)' : 'var(--g1)' }}>{e.status}</span>
                  <span style={{ fontWeight: 700, color: 'var(--cha)' }}>{(e.surname || '').toUpperCase()}, {e.given || ''} {e.suffix || ''}</span>
                  <span style={{ fontSize: 10, color: 'var(--mu)', fontFamily: "'JetBrains Mono',monospace" }}>{e.id}</span>
                  <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 10, fontWeight: 700, background: upd ? '#d1fae5' : '#fee2e2', color: upd ? '#065f46' : '#9b1c1c' }}>{upd ? '✅' : '⏳'}</span>
                </button>
              );
            })}
          </div>
